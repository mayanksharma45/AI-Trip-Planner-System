...
class GooglePlacesSearchTool:
    ...
    def google_search_restaurants(self, place: str) -> dict:
        """
        Searches for available restaurants in the specific place using GooglePlaces API.
        """
        return self.places_tool.run(f"what are the top 10 restaurants and entries in and around {place}?")
    
    def google_search_activity(self, place: str) -> dict:
        """
        Searches for popular activites in the specified place using GooglePlaces API."""
        return self.places_tool.run(f"Activities in and around {place}")
    
    def google_search_transportation(self, place: str) -> dict:
        """
        Searches for available modes of transportion in the specified place using GooglePlaces API"""
        return self.places_tool.run(f"What are the different modes of transportion available in {place}")
...
class TavilyPlaceSearchTool:
    ...
    def tavily_search_attractions(self, place: str) -> dict:
        """
        Searches for attrations in the specified place using TavilySearch.
        """
    ...
    def tavily_search_restaurants(self, place: str) -> dict:
        """
        Searches for availabe restaurants in the specified place using TavilySearch.
        """
        tavily_tool = TavilySearch(topic="general", include_answer="advanced")
    ...
    def tavily_search_transportation(self, place: str) -> dict:
        ...
        """
        tavily_tool = TavilySearch(topic="general", include_answer="advanced")
        result = tavily_tool.invoke({"query": f"what are the differnt modes of tranportations availble in {place}."})
        if isinstance(result, dict) and result.get("answer"):
            return result["answer"]
        return result
